...
![Equipment Profile](./images/watchdog_profile.webp)

The Watchdog driver is categorized as an  _Auxiliary_  driver. It can be found in the auxiliary category group in your favorite client.

## Options

![Options](./images/options.webp)

Before establishing connection, set the Mount & Dome driver names in  _Options_  tab. Some clients (e.g. KStars) would do this step automatically. If your observatory does not have a dome, simply leave it as the Dome Simulator.

## Main Control

![Main Control](./images/main_control.webp)

If the driver does not receive the _Heartbeat_ signal from the client after the timeout threshold expired, it starts the shutdown procedure. The shutdown procedure is currently composed of the following steps in order:
